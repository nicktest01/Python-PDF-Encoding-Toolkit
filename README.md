# Python-PDF-Encoding-Toolkit

## 🌟 Key Features
1. **編碼轉換器**：自動將 Big5 (繁體中文) 檔案轉換為 UTF-8，徹底解決亂碼問題。
   
   Automatically converts Big5 (Traditional Chinese) files to UTF-8, resolving encoding issues permanently.
   
3. **PDF 生成器**：將 TXT 內容轉換為 PDF，並完美支援中文顯示（不再需要手動處理複雜的 JAR 包）。
   
   Converts TXT content to PDF with full support for Chinese characters—eliminating the need for complex Java dependencies.

   
4. **網頁預覽器**：建立一個 Flask 微網頁，提供按鈕讓使用者直接在瀏覽器開啟 PDF。
   
   Built a Flask-based web interface allowing users to open and preview PDFs directly in the browser.

## 🌟 About This Project
Nick:
當年能力不足，為了處理Java編碼和 iText 庫，寫到頭破血流；

如今嘗試用Python解決當年的困境，由於是228連假抽空撰寫，所以沒有採用先進封裝寫法,只是單純快速撰寫完成,彌補當年的遺憾

Years ago, I struggled significantly with Java encoding issues and the iText library; at the time, it was a daunting technical challenge that felt like an uphill battle. Today, I’ve revisited and resolved those old frustrations using Python. This project was developed during the February 28th long weekend as a personal milestone. While I prioritized a quick, functional implementation over complex software patterns or advanced encapsulation, this serves as a meaningful way to close a chapter on a past technical regret.


## 🖥️ Preview

<img width="694" height="272" alt="image" src="https://github.com/user-attachments/assets/535c5e9e-0d27-4e48-9ed6-f039f3bc3e98" />
